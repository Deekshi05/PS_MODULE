...
from apps.psmodule.api.serializers import IndentSerializer
...
def get_acting_role(request) -> str:
    return (request.headers.get("X-Acting-Role") or "").strip().upper()


def get_actor_context(request) -> ActorContext:
    extrainfo = get_extrainfo_for_user(request.user)
    role = get_acting_role(request)
    if role == ActingRole.EMPLOYEE:
        return ActorContext(role=role, extrainfo=extrainfo)
    if role == ActingRole.DEPADMIN and has_designation(extrainfo, "depadmin"):
        return ActorContext(role=role, extrainfo=extrainfo)
    if role == ActingRole.PS_ADMIN and has_designation(extrainfo, "ps admin"):
        return ActorContext(role=role, extrainfo=extrainfo)
    if role == ActingRole.HOD and is_user_hod(extrainfo):
        return ActorContext(role=role, extrainfo=extrainfo)
    if role == ActingRole.REGISTRAR and has_designation(extrainfo, "registrar"):
        return ActorContext(role=role, extrainfo=extrainfo)
    if role == ActingRole.DIRECTOR and has_designation(extrainfo, "director"):
        return ActorContext(role=role, extrainfo=extrainfo)
